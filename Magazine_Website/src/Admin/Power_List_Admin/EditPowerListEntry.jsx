...
const EditPowerListEntry = () => {
  ...
  // Fetch power list and entry details
  const fetchData = async () => {
    try {
      ...
      // Fetch entry details
      const entryResponse = await api.get(`/lists/power-lists/entries/${entryId}`);
      ...
      setFormData({
        ...
        shortBio: entryData.shortBio || '',
        metaTitle: entryData.metaTitle || '',
        metaDescription: entryData.metaDescription || ''
      });
    } catch (error) {
      ...
      if (error.response?.status === 404) {
        ...
        showError('Failed to fetch entry details');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.put(`/lists/power-lists/entries/${entryId}`, submitData);
      showSuccess('Entry updated successfully');
      navigate(`/admin/power-lists/${id}/entries`);
    } catch (error) {
      console.error('Error updating entry:', error);
      if (error.response?.status === 403) {
        showError('Access denied: You do not have permission to edit this entry');
      } else {
      ...
      <div className={`min-h-screen ${bgMain} p-6`}>
        <div className="max-w-4xl mx-auto">
          <div className="text-center">
            <p className={`${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Entry not found</p>
            <Link to={`/admin/power-lists/${id}/entries`} className="text-blue-600 hover:underline mt-4 inline-block">
              Back to Entries
            </Link>
          </div>
        </div>
...
export default EditPowerListEntry;