...
const EditDownload = () => {
  ...
  const fetchDownload = async () => {
    try {
      setLoading(true);
      const response = await downloadService.getDownload(id);
      ...
      if (response.success) {
        ...
        setFormData({
          ...
          license: download.license || 'all_rights_reserved',
        ...
        toast.error('Failed to fetch download');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await downloadService.updateDownload(id, submitData);
      ...
      toast.error(error.message || 'Failed to update download');
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className={`min-h-screen ${bgMain} flex items-center justify-center`}>
        <div className="text-center">
          ...
          <p className="text-white text-xl">Loading download...</p>
        </div>
      </div>
    );
  }
    ...
    <div className={`min-h-screen ${bgMain} p-6`}>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className={`text-3xl font-bold ${textMain}`}>Edit Download</h1>
            <p className={`${isDark ? 'text-gray-300' : 'text-gray-600'} mt-2`}>
              Update download information and settings
            </p>
          </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'} mt-1`}>
                      Leave empty to keep current file. Supported formats: PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX, TXT, Images, Videos, Audio, Archives. Max size: 100MB
                    </div>
                    {formData.file && (
              ...
              <div className={`${cardBg} p-6 rounded-lg border`}>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      {saving ? 'Updating...' : 'Update Download'}
                    </button>
                  </div>
                </div>
              </div>
