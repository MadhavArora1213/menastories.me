...
const EditMediaKit = () => {
  ...
  const loadMediaKit = async () => {
    try {
      ...
      const response = await mediaKitService.getMediaKit(id);
      const mediaKit = response.data;

      setFormData({
        title: mediaKit.title || '',
        description: mediaKit.description || '',
        content: mediaKit.content || '',
        status: mediaKit.status || 'draft',
        featuredImage: null, // Don't preload file for editing
        imageCaption: mediaKit.imageCaption || '',
        imageAlt: mediaKit.imageAlt || '',
        metaTitle: mediaKit.metaTitle || '',
        metaDescription: mediaKit.metaDescription || '',
        keywords: mediaKit.keywords || []
      });
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await mediaKitService.updateMediaKit(id, submitData);
      navigate('/admin/media-kits');
    } catch (err) {
      console.error('Failed to update media kit:', err);
      setError(err.message || 'Failed to update media kit');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="edit-media-kit">
      {/* Header */}
      <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Edit Media Kit</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Update media kit information and settings
          </p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            {loading ? 'Updating...' : 'Update Media Kit'}
          </button>
        </div>
      </div>
