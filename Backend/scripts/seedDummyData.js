const { Article, Category, Tag, Admin, ArticleCategory, ArticleTag, sequelize } = require('../models');
...
const seedDummyData = async () => {
  try {
    ...
    const articleAssociations = [
      // Article 1 (AI) - Technology category, AI and Innovation tags
      { articleId: 'art-001', categoryId: 'cat-001', tagIds: ['tag-001', 'tag-002'] },
      // Article 2 (Market) - Business category, Market Analysis tag
      { articleId: 'art-002', categoryId: 'cat-002', tagIds: ['tag-003'] },
      // Article 3 (Celebrity) - Entertainment category, Celebrity tag
      { articleId: 'art-003', categoryId: 'cat-003', tagIds: ['tag-004'] },
      // Article 4 (Football) - Sports category, Football tag
      { articleId: 'art-004', categoryId: 'cat-004', tagIds: ['tag-005'] },
      // Article 5 (Tech Merger) - Technology and Business categories, Breaking News tag
      { articleId: 'art-005', categoryId: 'cat-001', tagIds: ['tag-006'] }
    ];

    for (const assoc of articleAssociations) {
      // Associate with category
      await ArticleCategory.create({
        articleId: assoc.articleId,
        categoryId: assoc.categoryId
      });

      // Associate with tags
      for (const tagId of assoc.tagIds) {
    ...
    console.log('✓ Article associations created');
